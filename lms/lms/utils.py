...
def get_mentors(course):
    ...
    mentors = frappe.get_all(
        "LMS Course Mentor Mapping", {"course": course}, ["mentor"]
    )
...
def get_signup_optin_checks():
    ...
    for check in checks:
        if frappe.db.get_single_value("LMS Settings", check):
            ...
            links.append(
                "<a href='/" + route + "'>" + mapper[check].get("title") + "</a>"
            )
...
def is_onboarding_complete():
    ...
    return {
        ...
        "first_course": (
            frappe.get_all("LMS Course", limit=1, order_by=None, pluck="name")[0]
            if course_created
            else None
        ),
    }
...
def get_course_details(course):
    ...
    course_details.courses = frappe.get_all(
        "Chapter Reference", filters={"parent": course}, fields=["chapter"]
    )
...
def get_batch_details(batch):
    ...
    if batch_details.seat_count:
        batch_details.seats_left = batch_details.seat_count - len(
    ...
    for student in students_list:
        ...
        for course in batch_courses:
            progress = frappe.db.get_value(
                "LMS Enrollment",
                {"course": course, "member": student.student},
                "progress",
            )
...
def get_course_students(course):
    ...
    course_settings = frappe.get_value("LMS Course", course, ["where_you_can_see_your_own_students", "where_you_can_see_all_students"], as_dict=True)
