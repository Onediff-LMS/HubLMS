					...
					<div class="flex flex-col space-y-4 text-sm text-gray-800">
						...
						<Tooltip :text="__('Date')">
							<div class="flex items-center space-x-2 w-fit">
								...
								<span>
									{{ dayjs(event.creation).format('DD MMM YYYY') }}
								</span>
							</div>
						</Tooltip>
					...
					<template #default="{ tab }">
						<div v-if="tab.label == 'Evaluation'" class="flex flex-col space-y-4 p-5">
							<div class="flex items-center justify-between">
								...
								<FormControl type="select" :options="statusOptions" v-model="evaluation.status"
									:label="__('Status')" class="w-1/2" />
							</div>
							<Textarea v-model="evaluation.summary" :label="__('Summary')" :rows="7" />
						...
						<div v-else class="flex flex-col space-y-4 p-5">
							...
							<Link v-model="certificate.template" :label="__('Template')" doctype="Print Format"
								:filters="{
